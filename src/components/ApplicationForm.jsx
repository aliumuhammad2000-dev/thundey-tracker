...
function ApplicationForm({ application, onClose, onSubmit, saving }) {
  const [form, setForm] = useState(application ? { company: application.company, role: application.role, status: application.status, notes: application.notes } : initialForm)
  ...
  return (
    <div className="fixed inset-0 z-10 flex items-center justify-center bg-slate-950/80 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="form-title">
      <form className="max-h-[calc(100vh-2rem)] w-full max-w-lg overflow-y-auto rounded-2xl border border-slate-700 bg-slate-900 p-5 shadow-2xl sm:p-6" onSubmit={handleSubmit}>
        <div className="flex items-start justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-cyan-400">{application ? 'Update entry' : 'New entry'}</p><h2 className="mt-2 text-xl font-semibold" id="form-title">{application ? 'Edit application' : 'Add application'}</h2></div><button aria-label="Close form" className="rounded-lg p-2 text-slate-400 hover:bg-slate-800 hover:text-slate-100" onClick={onClose} type="button"><X size={18} /></button></div>
        ...
        <button className="mt-6 w-full rounded-xl bg-cyan-400 px-4 py-3 font-semibold text-slate-950 transition hover:bg-cyan-300 disabled:cursor-not-allowed disabled:opacity-60" disabled={saving} type="submit">{saving ? 'Saving...' : application ? 'Update application' : 'Save application'}</button>
      </form>
    </div>
  )
}
