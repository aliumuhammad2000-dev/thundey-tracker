...
function ApplicationsPanel({ applications, deletingId, error, loading, onAdd, onDelete, onEdit }) {
  ...
  const filteredApplications = applications.filter((application) => {
    const matchesSearch = `${application.company} ${application.role}`.toLowerCase().includes(searchTerm.toLowerCase())
    const matchesStatus = statusFilter === 'all' || application.status.toLowerCase() === statusFilter
    return matchesSearch && matchesStatus
  })

  return (
    <section className="mt-8 min-w-0 overflow-hidden rounded-2xl border border-slate-800 bg-slate-900/60" id="applications">
      <div className="flex flex-col gap-4 border-b border-slate-800 p-5 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex w-full flex-col gap-3 sm:w-auto sm:flex-row">
          <input aria-label="Search applications" className="w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-slate-200 outline-none placeholder:text-slate-600 focus:border-cyan-400 sm:w-52" onChange={(event) => setSearchTerm(event.target.value)} placeholder="Search applications" type="search" value={searchTerm} />
          <div className="relative w-full sm:w-auto">
            <select aria-label="Filter applications by status" className="w-full appearance-none rounded-lg border border-slate-700 bg-slate-900 py-2 pl-3 pr-9 text-sm font-medium text-slate-300 outline-none transition hover:border-slate-500 focus:border-cyan-400 focus:ring-2 focus:ring-cyan-400/20 sm:w-36" onChange={(event) => setStatusFilter(event.target.value)} value={statusFilter}>
              ...
              <option className="bg-slate-900 text-slate-200" value="rejected">Rejected</option>
            </select>
            ...
            <div className="px-5 py-16 text-center sm:py-24">
              ...
              {!loading && !error && filteredApplications.length > 0 && <div className="grid gap-3 text-left sm:grid-cols-2">
                {filteredApplications.map((application) => <article className="min-w-0 overflow-hidden rounded-xl border border-slate-800 bg-slate-950/70 p-4" key={application.id}><div className="flex min-w-0 items-start justify-between gap-3"><div className="min-w-0"><h3 className="truncate font-medium text-slate-100">{application.role}</h3><p className="mt-1 truncate text-sm text-slate-400">{application.company}</p></div><span className="shrink-0 rounded-full bg-cyan-400/10 px-2.5 py-1 text-xs text-cyan-300">{application.status}</span></div><p className="mt-3 truncate text-xs text-slate-600">{application.notes}</p><div className="mt-4 flex gap-4"><button aria-label={`Edit ${application.role} at ${application.company}`} className="inline-flex items-center gap-2 text-xs text-slate-500 transition hover:text-cyan-300" onClick={() => onEdit(application)} type="button"><Pencil size={14} />Edit</button><button aria-label={`Delete ${application.role} at ${application.company}`} className="inline-flex items-center gap-2 text-xs text-slate-500 transition hover:text-rose-300 disabled:cursor-not-allowed disabled:opacity-50" disabled={deletingId === application.id} onClick={() => onDelete(application.id)} type="button"><Trash2 size={14} />{deletingId === application.id ? 'Deleting...' : 'Delete'}</button></div></article>)}
              </div>}
        ...
        <p className="mx-auto mt-2 max-w-sm text-sm leading-relaxed text-slate-500">When you add an application, you'll be able to track its status, notes, and next steps from this dashboard.</p>
              ...
              {!loading && !error && applications.length > 0 && filteredApplications.length === 0 && <p className="text-sm text-slate-500">No applications match your search.</p>}
      </div>
    </section>
  )
}
